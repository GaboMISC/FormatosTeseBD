...
-- Consulta General
SELECT HA.Total AS TotalHojaDeAnalisis, EV.Calificacion, CONCAT(PR.Letra, ', ', PR.Descripcion) AS Prioridad,
...
FROM Proyecto AS PY
	...
	INNER JOIN Evaluadores AS ES ON ES.NumEvaluacion = EV.Id
	INNER JOIN CargoPersona AS CP ON CP.Id = ES.NumCargoPersona
	INNER JOIN Persona AS PE ON PE.Id = CP.NumPersona;